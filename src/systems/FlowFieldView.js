...
export class FlowFieldView {
  ...
  update() {
    ...
    // Build phase only. Once creeps are on the board they ARE the visualisation,
    // and a lit field under a fight is just noise.
    if (!this.enabled || !city.flow.ready || (creeps && creeps.creeps.length > 0)) {
      if (this.mesh) this.mesh.visible = false
      return
    }
    const key = city.flow.version
  ...
  _rebuild() {
    ...
    let count = 0
    for (let i = 0; i < W * H; i++) if (dist[i] >= 1 && (dx[i] || dz[i])) count++
    ...
    let longest = 1
    for (let i = 0; i < W * H; i++) if (dist[i] > longest) longest = dist[i]

    const dummy = this._dummy
    // Dark orange for the whole field. This drops the blue/green split that
    // showed king-bound cells against generator-bound ones - say the word and it
    // can come back as two shades of orange instead of two hues.
    const kingCol = FLOW_COLOR
    const genCol = FLOW_COLOR
    let k = 0
    for (let gy = 0; gy < H; gy++) {
      for (let gx = 0; gx < W; gx++) {
        const i = gy * W + gx
        if (dist[i] < 1 || (!dx[i] && !dz[i])) continue
