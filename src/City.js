...
import { uniform, cos, sin, vec3, normalWorld, positionViewDirection, cameraViewMatrix, roughness, pmremTexture, mrt, min, output } from 'three/tsl'
...
export class City {
  ...
  constructor(scene, params) {
    ...
    // The beam, ring and cube that say where the king is (systems/KingVisuals).
    this.kingVisuals = new KingVisuals(this)
    // The cell grid, lot grid, dots and board outline (systems/BoardGrid).
    this.grid = new BoardGrid(this)
  }
  ...
  growPlayArea() {
    ...
    this.grid.expandGrids(prevHalf)
   ...
   */
  flashBarrier(owner) {
    const mesh = owner.king ? this.kingVisuals.kingRing
      : this.rangeVisuals?.shield.meshes.get(owner)
    if (!mesh) return
  ...
  triggerGameOver() {
    ...
    this.kingVisuals.kingDeathPulse()
    this.onGameOver?.()
  }
