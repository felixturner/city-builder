...
import { DemoUI } from './systems/DemoUI.js'
...
export class Demo {
  ...
  async init() {
    ...
    // Pause and fast-forward chips, the game-over panel and the menu.
    this.ui = new DemoUI(this)
    this.ui.build()
    ...
    if (this.started && !this.paused && !this.isGameOver && !this.tabHidden) {
      ...
      for (let i = 0; i < steps; i++) {
        ...
        if (this.kingDead && !this.isGameOver) {
          ...
          if (this.gameOverDelay <= 0) this.ui.showGameOver()
        }
      }
    }
